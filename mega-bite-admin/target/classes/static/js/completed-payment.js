...
$('#completed-payment-table').on('draw.dt', function () {
    $('[data-toggle="tooltip"]').tooltip();
});

function drawDataTable() {
    $('table#completed-payment-table').DataTable().clear().destroy();
    $('table#completed-payment-table').DataTable({
        responsive: true,
        ajax: {
            'contentType': 'application/json',
            'url': '/payment/dataTable?type=' + 'completed',
            'type': 'POST',
            'data': function (d) {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return '<span class="d-flex justify-content-center">'
                        + '<a href="/completed-payments/' + data + '/detail"'
