...
$('#debit-payment-table').on('draw.dt', function () {
    $('[data-toggle="tooltip"]').tooltip();
});

function drawDataTable() {
    $('table#debit-payment-table').DataTable().clear().destroy();
    table = $('table#debit-payment-table').DataTable({
        responsive: true,
        ajax: {
            'contentType': 'application/json',
            'url': '/payment/dataTable?type=' + 'debit',
            'type': 'POST',
            'data': function (d) {
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return '<span class="d-flex justify-content-center">'
                        + '<a href="/debit-payments/' + data + '/detail"'
